...
- **复制剪贴板**: `navigator.clipboard` 仅在安全上下文(HTTPS/localhost)可用。管理员常通过局域网 IP(http://192.168.x.x)访问，此时它不存在会抛异常、且 writeText 的 Promise 被拒时也没有反馈，导致「路由配置」页复制(请求地址/模型名/API Key)看起来无效。Routing.tsx 的 `copyText` 已改为：优先 clipboard，不可用或被拒时回退 `document.execCommand('copy')`(隐藏 textarea 方案)，成功与否都有明确 message 反馈
...
- **UsageTrend.tsx**: 用量趋势页，堆叠柱状图 `Column`（`@ant-design/charts`）展示每日 token 消耗，支持两种分组模式（默认「按队列」）：「按队列」走 `/stats/daily/queues`（按 `routed_model` 分组，系列 = 队列名，一个队列可对应多个模型）；「按模型」走 `/stats/daily/models`（按 `model+provider` 分组，系列 key = `模型 · 供应商`）。tooltip 用 `customContent` 自定义显示系列名+千分位(`formatThousands`，`toLocaleString`)+当天汇总行；筛选区服务商下拉 + 模型下拉（模型下拉 `value` 用 `model.id` 避免同名模型重复，label 未选供应商时带 `(供应商名)`，选了供应商只显示该供应商模型）；时间范围(7/14/30/90天)、指标切换(总Token/输入/输出/请求数/缓存命中)；统计卡片走 `/stats/daily`，图表按模式走对应接口；x 轴日期显示 `YYYY-MM-DD`（后端 `DATE_FORMAT`/`date(created_at)` 返回，`autoRotate`/`autoHide` 防重叠）；前端用 `formatDate` 生成近 `days` 天完整日期序列，并为每个日期×系列生成数据点（无数据的系列补 0 值，即「稠密数据点」），保证无数据的天在 x 轴占据空位、且图例筛选某个系列后空日期也不会消失（若用单一 fallback 占位点会随该系列被筛选而丢失日期位置）；图表卡片另加 `usage-trend-card` 类以禁用 `aurora-chart-card` 的居中光晕伪元素(`.usage-trend-card .ant-card-body::before { display:none }`)，因为该淡绿羽化光晕是为 Dashboard 饼图设计的，柱状图背景下中心会浮现一个绿点
